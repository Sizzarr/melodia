...
async function main() {
  ...
  // Deploy KYCRegistry
  const KYCRegistryFactory = new ethers.ContractFactory(kycABI, kycBin, deployer);
  ...
  // Deploy MusicIPNFT
  const MusicIPNFTFactory = new ethers.ContractFactory(nftABI, nftBin, deployer);
  const musicIPNFT = await MusicIPNFTFactory.deploy();
